...
DATA_DIR = os.path.join(_PROJECT_ROOT, "data", "experiment_control")

# This will run Trials 1-10 for State 9 only.
TARGET_STATE = 9       
TRIALS_PER_STATE = 10   

# Randomization Limits (for Trials 5-7)
...
def get_next_task(user_name):
    ...
    # Check trials 1 through 7 for this state
...
def run_trial(user, state, trial):
    ...
    # TRIALS 3-6: Practice (Manual - Control Group)
    # Note: Control group gets full control (Manual) during practice
    elif 3 <= trial <= 6:
        use_shared_autonomy = False
        mode_description = "PRACTICE (Manual - Straight)"
    ...
    file_base = f"control_{user}_state{state}_trial{trial}_sa{use_shared_autonomy}"
    ...
    # Only print offsets if we are actually randomizing (Trials 9-10)
    if trial >= 9:
